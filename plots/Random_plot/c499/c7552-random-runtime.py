...
for j in range(1, 73):         #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!need change if use other circuit
    i = "c499-mux4-1-XOR-" + str(j) + ".v"
# create file

    make_cmmd = " python ./XOR_Cam_randmom.py c499-mux4-1.v " + str(j)
...
# check EC and import to a txt
    ...
    with open("time_info_XOR", "a") as outfile:
        outfile.write(i.strip("c499-mux4-1-XOR-").strip(".v") + "                    ")
    with open("temp-time", "r") as infile:
        timeline = infile.read().split("\n")
