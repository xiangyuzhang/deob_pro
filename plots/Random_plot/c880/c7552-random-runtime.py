...
for i in cam_list:
    cmmd = "/usr/bin/time -v -o temp-time ./minisat-incre-simp decam-incre c880-abcmap-fmt.v " + i
...
# check EC and import to a txt
    ...
    res = EC("c880-abcmap-fmt.v", i, "finalSolu.log")
    if (res == 1):
    ...
    with open("time_info_original", "a") as outfile:
        outfile.write(i.strip("c880-mux4-").strip(".v") + "                    ")
...
for j in range(1, 86):         #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!need change if use other circuit
    i = "c880-mux4-1-XOR-" + str(j) + ".v"
# create file

    make_cmmd = " python ./XOR_Cam_randmom.py c880-mux4-1.v " + str(j)
...
# check EC and import to a txt
    ...
    with open("time_info_XOR", "a") as outfile:
        outfile.write(i.strip("c880-mux4-1-XOR-").strip(".v") + "                    ")
    with open("temp-time", "r") as infile:
        timeline = infile.read().split("\n")
