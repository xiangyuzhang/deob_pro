...
class buildCamCkt:
	...
	def __init__(self, expCmdGateNum, OracInPath):
...
\n  inv1  g4(.a(s1), .O(n9));\n  nand2 g5(.a(n9), .b(n8), .O(n10));\n  nor2  g6(.a(n10), .b(n7), .O(n11));\n  inv1  g7(.a(pi11), .O(n12));\n  inv1  g8(.a(pi12), .O(n13));\n  nand2 g9(.a(n13), .b(n12), .O(n14));\n  nand2 g10(.a(s1), .b(n8), .O(n15));\n  nor2  g11(.a(n15), .b(n14), .O(n16));\n  nor2  g12(.a(n16), .b(n11), .O(n17));\n  nand2 g13(.a(n17), .b(n6), .O(c1))']
	...
	def _normalIteration(self, gateNum):
		...
		#print 'This program camouflages all gates. Begin to camouflage ... ...'
		self._updateCNFV()           
	...
	def _main(self):
		...
		self._preprocessing()
		camGateLog = []
		candiType = ['nand2', 'nor2', 'xor2']
		endLoop = 0
		while len(camGateLog) != self.expCmdGateNum:	
			selGateNum = random.randrange(1, maxInt+1)
			if selGateNum in camGateLog: # this gate is selected
				continue
				"""
				endLoop += 1
				# if for 10000 consecutive times, selGateNum is always a num camd previously
				if endLoop==10000:
					break
				else: 
					continue
				...
				gateLine = self.orgV[selGateNum].replace('\n','')
				if gateLine != "":
					#print gateLine
					orgVgateType = re.search(r'^(.*)(?=g\S+\(\.)', gateLine.replace(' ','')).group()
					if orgVgateType in candiType:
						#endLoop = 0
						camGateLog.append(selGateNum)
						self._normalIteration(selGateNum)
						print orgVgateType, 'gate', self.tmpGateNum, 'is camouflaged.'
					else:
						print 'gate', selGateNum,"is not one of nand2, nor2, xor2. Passed."
		...
		totCamdGate = len(camGateLog)
		return (camdCkt, inFilePath, totCamdGate)
...
def randGenCam():
	...
	print "\n\t##########  Welcome to randCam  system.  ##########"
	print "\t#This system is used to randomly select a gate fr-#"
	print "\t#-om circuit and camouflage it by a camflaging mo-#"
	print "\t#-dule.                                           #"
	print "\t###################################################"
	

	parser = argparse.ArgumentParser(usage='python buildCamCkt-randCam-cmmdline.py [-h] <orac.v> [num]', description='',)
	...
	if not os.path.isfile(OracInPath):
		...
		return
	camCkt = buildCamCkt(expCmdGateNum, OracInPath)
	camedTup = camCkt._main()
	camedCkt = camedTup[0]
	inFilePath = camedTup[1]
	totCamdGate = camedTup[2]
	outFilePath = re.search(r'(.*)(?=\.)', inFilePath).group()+'-randCam'+str(expCmdGateNum)+'.v'
	#print camedCkt
	camedCkt.append('\nendmodule')
	#print camedCkt
	outxt = (';\n').join(camedCkt)
	with open(outFilePath, 'w') as outfile:
		outfile.write(outxt)
		print 'Result is written in', outFilePath
	return totCamdGate


if __name__=='__main__':
	totCamdGate = randGenCam()
