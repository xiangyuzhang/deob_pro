...
while(1):
    for Num_mux in range(1, 11):
        Mux_circuit = "c432-mux4-"+ str(Num_mux) + ".v"
        XOR_circuit = "c432-mux4-"+ str(Num_mux) + "-XOR-" + str(43-Num_mux*4) + ".v"
        make_cmmd = "python XOR_Cam_random.py " + Mux_circuit + " " + str(43-Num_mux*4)
        call(make_cmmd, shell = True)
    # run
        run_cmmd = "/usr/bin/time -v -o temp-time ./minisat-incre-simp decam-incre c432-abcmap-fmt.v " + XOR_circuit
        call(run_cmmd, shell = True)
        #os.rename("increIterationInfo.log", XOR_circuit.strip(".v") + "-interationInfo.dat")
    # EC
        res = EC("c432-abcmap-fmt.v", XOR_circuit, "finalSolu.log")
        if (res == 1):
    ...
    # collect data
        ...
        for line in timeline:
            if "User time (seconds):" in line:
                ...
                break
        with open("time-info", "a") as outfile:
            outfile.write(str(Num_mux) + "                   " + str(43-Num_mux) + "                    " + time_info +"\n")
    # plot
