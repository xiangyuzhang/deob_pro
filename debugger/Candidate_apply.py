...
parser = argparse.ArgumentParser(usage='python Candidate [-h] <cam.v> <Candidate>[number]', description='This program will use '
                                                                                               'cam.v to generate a evener '
                                                                                               'harder circuit.', )
parser.add_argument('<cam.v>', help='input cam circuit that will be made harder')
parser.add_argument('<Candidate>', help='input candidate that will be applied')
parser.add_argument('number', action='store', type=int, help='define which specific line applied in candidate list')

args = parser.parse_args()
Num_XOR = args.number
camIn = sys.argv[1]
candidateIn = sys.argv[2]
camInPath = os.path.abspath(camIn)
candidateInPath = os.path.abspath(candidateIn)
if not os.path.isfile(camInPath):
    print 'Invalid cam circuit file!!!!\n'
if not os.path.isfile(candidateInPath):
    print 'Invalid candidate file!!!!\n'
Camfile = camInPath

with open (candidateInPath, "r") as infile:
    random_candiate_list = infile.read().split('\n')
random_sequence = random_candiate_list[Num_XOR-1].split()
...
random.random()
# random.seed(1)
 # this is the list store the srand sequence
random_couter = 0
...
for line in Vlines:
    if 'input' in line and '//' not in line:
        ...
        for i in random_sequence:
            line = line.replace(i+",", i+"_new,")
            PI_info = line
            if i not in replaced_PIPO:
                replaced_PIPO.append(i)
        break
...
for i in replaced_PIPO:
    with open("Candidate_applied", "a") as outfile:
        outfile.write(i + " ")
with open("Candidate_applied", "a") as outfile:
    outfile.write("\n")
