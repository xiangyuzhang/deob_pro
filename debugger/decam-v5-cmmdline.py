...
class DissectCkt:
    ...
    def __init__(self, OracIn, CamIn, MuxStyle, verbose):
    ...
    def _initDissect(self):
        ...
        if self.verbose==True:
            with open('nonIncreIterationInfo.log', 'r') as file2parse:
                solveInterSolu = file2parse.read()
                solveInterSoluData = solveInterSolu.replace('\r','').split('\n')
                for line in solveInterSoluData:
                    if "Number of variables" in line:
                        self.varLs.append( line.replace(' ','').split(':')[1].strip('|') )
                    elif "Number of clauses" in line:
                        self.clauseLs.append( line.replace(' ','').split(':')[1].strip('|') ) 
        satTime2=time.time()
    ...
    def _findSolu(self):
        ...
        if num2dup-1 > 0:
            ...
            #2. p-bit constraints (all connected together):  laiiysydjf
        ...
        if initRes==1:
            ...
            if self.verbose==True:
                ...
                for i in iterTimeRec:
                    res += str(i)+'\t'+str(self.varLs[i-1])+"\t"+str(self.clauseLs[i-1])+'\t'+str(iterTimeRec[i])+'\n'
                res += "The last iteration is UNSAT.\n"
                return res
            else:
                return   
        else:
...
def rundecam():
    ...
    parser = argparse.ArgumentParser(usage='python decam-v5-cmmdline.py [-h] <orac.v> <cam.v> [-forbid][-o <res_file>]', description='This program will use orac.v and cam.v to generate a programming bit vector (if exists)\
    ...
    parser.add_argument('--forbid',action = 'append', type=str,help='define the forbidden programming bits for MUX')
    parser.add_argument('-o', "--res_file", nargs='?', type=argparse.FileType('w'), help='output file which stores iteration times.')
    ...
    # Here I add EC #
    findSoluFilelog = re.search(r'(.*)(?=\.)', CamInPath).group()+'-_findSolu'+'.log'
